/*--------------------------------------------------------------------------------
	(c)1995-2000, PolarSoft(r) Inc. and National Institute for Standards and Technology
	
module:		BACPRIM32.C
desc:		BACnet Encoding/Decoding Primitives DLL v2.37
authors:	David M. Fisher, Jack R. Neyer
last edit:	29-Jan-01 [238] JJB revise for C++, added PICS namespace and typecasts
			03-Mar-00 [237] DMF fix empty bitstring case for atoeBITSTRING
			15-Dec-99 [236] DMF 
	...
	{	if (hcrit==NULL) return 0;
		...
		{	if ((word)(op-iop+2)>opmax) return 0;	//account for outer tags
			*op++=0x1E;								//open tag 1 list of selection criteria
			for (i=0;i<nv;i++)
			{	GetListText(hcrit,i,(char *)p);		//get the selection criterium text to encode
				if (c=eSelCrit(op,(word)(opmax-(op-iop)),(char *)p))
					op+=c;
				else
					return 0;						//can't encode it		
			}
